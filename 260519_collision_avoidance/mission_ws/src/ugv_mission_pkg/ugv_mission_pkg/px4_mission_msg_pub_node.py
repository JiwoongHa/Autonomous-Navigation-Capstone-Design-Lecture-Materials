...
class Px4MissionMsgPubNode(Node):
    ...
    def __init__(self):
        ...
        # Each entry: {'u_ref': float, 'psi_ref': float,
        #              'received': bool, 'stamp_ns': int}
        self._cmd = {}
        ...
        # Subscribe to every mission topic automatically
        for name, cfg in self.MISSION_MODES.items():
            # Use a closure to capture 'name' correctly in the lambda
    ...
    #                        Helper Functions
    # =================================================================
    def _is_cmd_fresh(self, name: str) -> bool:
        """Returns True if the command from 'name' was received within timeout."""
        state = self._cmd[name]
        if not state['received'] or state['stamp_ns'] == 0:
            return False
    ...
    def _timer_cb(self):
        ...
        # ── Priority scan: first matching mode wins ──────────────────
        for name, cfg in self.MISSION_MODES.items():
